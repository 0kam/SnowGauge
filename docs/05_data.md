...
| `tilt_deg` | センサ光軸の傾き（真下方向からの角度）。`pitch_deg` は基板面内（電池端子側へ +）、`roll_deg` は基板面外（部品面側へ +）の成分。基板は筐体内で鉛直（USB 左・センサ端子 J2 上）に取り付ける前提。定義と図は [coordinate_system.md](coordinate_system.md) |
