...
- 設計仕様書（要件・設計判断・改版履歴）: [SnowGauge_設計仕様書_v0.15.md](SnowGauge_設計仕様書_v0.15.md)
...
基板 v1.2 発注済み（到着 9/10 ごろ）。ブレッドボードでファームウェア（ステップ 1〜4）とアプリの実機確認まで完了。TSD20 版ファームウェア（ステップ 5）は実装・ビルド済みで、ブレッドボード（U2 を F33 に差し替え）での実機確認待ち。次は基板到着後の再測定、筐体。
