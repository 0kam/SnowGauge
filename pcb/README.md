...
## Component orientation (U1, U2, Q1, Q2)

The KiCad TO-220 / TO-251 vertical footprints put the reference designator on the **metal-tab side**, so the part's **marked (printed) face points the other way**. With the board held "SnowGauge v1.2" up / J1 down: U1 and Q1 have the tab towards the top edge (print facing the XIAO), U2 and Q2 are rotated 180 deg (tab towards the board centre, print facing the top edge). Assembly instructions with a figure: [docs/02_assembly.md](../docs/02_assembly.md) B-2 (figure regenerated by `docs/assets/figures/gen_component_orientation.py`).
